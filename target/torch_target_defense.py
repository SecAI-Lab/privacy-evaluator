...
def load_torch_cifar100(train=True):
    ...
    dataset = torchvision.datasets.CIFAR100(
        root='../data/datasets/cifar100-data', train=train, download=True, transform=transform)
...
def run(args):
    ...
    save_dir = f"models/{args.datasets}_{args.model_name}/{args.defense}"
    data_dir = f"models/"
    ...
    # confidence_masking
        ...
        count = 0
        for epoch in range(args.epochs):
            train_acc, train_loss = model.train(target_train_loader, f"epoch {epoch} train")
            val_acc, val_loss = model.test(target_valid_loader, f"epoch {epoch} valid")
            test_acc, test_loss = model.test(target_test_loader, f"epoch {epoch} test")
            if val_acc > best_acc:
                best_acc = val_acc
